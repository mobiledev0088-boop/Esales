...
export default function TargetASETerritory() {
  ...
  const mergeData = useMemo((): SummaryItem[] => {
    ...
    const territoryMap = new Map<string, SummaryItem>();

    ProductCategory.forEach((item: ProductCategory) => {
      const territoryKey = item.Territory?.toLowerCase()?.trim();
      if (!territoryKey) return; // Skip if territory name is invalid
      ...
      const node = territoryMap.get(territoryKey)!;
    ...
    groupedList = Array.from(territoryMap.values()).sort((a, b) =>
  ...
  const renderItem = useCallback(
    ({item}: {item: SummaryItem}) => {
      ...
      const handleLegendPress = (type: string, Territory: string) => {
        // Optional: Implement any interaction when legend is pressed
        console.log('Legend pressed:', type, Territory);
              ...
              <View className="flex-1 flex-row items-start gap-3 py-3">
                ...
                <View className="flex-1 ">
                  <AppText
                    ...
                    numberOfLines={1}
                    className="mb-2">
                    {item.Territory || 'Unknown Territory'}
                  </AppText>
          ...
          {hasProducts ? (
            <>
              ...
              <View className="items-center mt-2">
                ...
                />
                {/* Legend */}
                <View className="flex-row flex-wrap justify-center mt-5 gap-3 px-4 mb-4">
                  {item.PartnerWiseDetails.map((p, idx) => (
                    <TouchableOpacity
                      onPress={() =>
                        handleLegendPress(p.ALP_Type, item.Territory)
                      }
            ...
            <View className="py-8 items-center">
              <AppText size="sm" className="text-gray-400">
                No products available for this territory.
              </AppText>
            </View>
          )}
        </Accordion>
  ...
  const keyExtractor = useCallback(
    (item: SummaryItem, index: number) => `${item.Territory}_${index}`,
    [],
  );
      ...
      <View className="flex-1 items-center justify-center py-20 px-6">
        <View className="items-center bg-white dark:bg-darkCard rounded-2xl p-8 shadow-sm">
          ...
          <AppText size="sm" className="text-gray-500 text-center">
            {isError
              ? 'Unable to fetch territory data. Please try again later.'
              : 'No territory data available for the selected period.'}
          </AppText>
          {isError && (
            <TouchableOpacity
              onPress={() => refetch()}
  ...
  return (
    <AppLayout title="Target ASE Territory" needBack needScroll={false}>
      {!isLoading && (
