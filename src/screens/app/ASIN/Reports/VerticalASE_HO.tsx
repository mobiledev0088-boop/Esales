...
// -------------------- API Hook --------------------
const useGetTrgtVsAchvPartnerTypeWise = (YearQtr: string, AlpType: string) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await handleASINApiCall(
        ...
        {employeeCode, RoleId, YearQtr, AlpType},
      );
...
const QuantityChart = memo(({partner}: {partner: PartnerASEData}) => {
...
const PartnerCard = memo(({partner}: {partner: PartnerASEData}) => {
        ...
        <View className="flex-row">
          ...
          <View className="flex-1 pl-2">
            ...
            <AppText
              ...
              numberOfLines={1}
              className={
                achievementPercent >= 100 ? 'text-green-600' : 'text-orange-600'
              }>
              {achievementPercent}%
            </AppText>
          </View>
...
export default function VerticalASE_HO() {
  ...
  const renderItem = useCallback(
    ({item}: {item: PartnerASEData}) => <PartnerCard partner={item} />,
    [],
  );
