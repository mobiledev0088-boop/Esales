...
export interface DemoAWPFilterPayload {
  filters: {
    category: string;
    agpName: string;
    status: string;
...
type FilterGroup = 'category' | 'agpName' | 'status' | 'demoType';
...
// Searchable list component for AGP Name
      ...
      <View className="flex-1">
        <View className="px-3 py-2 border-b border-slate-200 dark:border-slate-600">
          <View className="flex-row items-center bg-slate-100 dark:bg-slate-700 rounded-lg px-3 py-2">
            <AppIcon name="search" type="feather" size={16} color="#9CA3AF" />
            <TextInput
...
export default function DemoAWP_Filter_Sheet() {
  const payload = (useSheetPayload?.() || {}) as DemoAWPFilterPayload;
  ...
  const groups = useMemo(
    () => [
      ...
      {
        key: 'agpName' as FilterGroup,
        label: 'AGP Name',
        hasValue: !!filters.agpName,
      },
      {
    ...
    [filters.category, filters.agpName, filters.status, filters.demoType],
  );
  ...
  const rightPanel = () => {
    ...
    switch (group) {
      ...
      case 'agpName': {
        const hasSelection = !!filters.agpName;
        return (
          <View className="flex-1">
            <SearchableList
              data={options.agpOptions}
              selectedValue={filters.agpName}
              onSelect={value =>
                setFilters(prev => ({...prev, agpName: value}))
              }
              searchPlaceholder="Search AGP Name..."
            />
            {clearSelectionCTA(hasSelection, () =>
              setFilters(prev => ({...prev, agpName: ''})),
            )}
          </View>
        );
      }
...
export const showDemoAWPFilterSheet = (props: DemoAWPFilterPayload) =>
  SheetManager.show('DemoAWPFilterSheet', {payload: props});

const hideDemoAWPFilterSheet = () => SheetManager.hide('DemoAWPFilterSheet');
