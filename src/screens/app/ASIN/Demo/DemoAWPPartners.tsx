...
const useGetPartnerDemoData = (
  ...
  return useQuery({
    queryKey: ['partnerDemoDataAWP', employeeCode, YearQtr],
    enabled,
    queryFn: async () => {
        ...
        <ScrollView
          ...
          <View className="mb-4">
            ...
            <View className="flex-row flex-wrap gap-2 mt-1">
              <View
                className={`px-2.5 py-1 rounded-md ${isPending ? 'bg-amber-500/10 dark:bg-amber-500/20' : 'bg-emerald-500/10 dark:bg-emerald-500/20'}`}>
                <AppText
                  size="xs"
                  weight="semibold"
          ...
          <View className="mb-3">
            ...
            {isLoadingHub && (
              <View className="space-y-3">
                {[...Array(3)].map((_, i) => (
                  <View key={i} className="bg-slate-50 dark:bg-slate-700 rounded-xl p-4 mb-3">
                    <View className="mb-2">
...
// ================= Main Component =======================
export default function DemoAWPPartners() {
