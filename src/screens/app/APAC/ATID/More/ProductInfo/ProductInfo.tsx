...
import {Disclaimer} from '../../../../ASIN/More/ActivatedDetails/component';
...
export default function ProductInfo() {
  ...
  const {mutate, data, isPending} = useGetProductList();

  const hasActiveFilter = useMemo(
    () => expandableRAM || expandableStorage,
    [expandableRAM, expandableStorage],
  );
  ...
  const toggleFilter = useCallback(
    (type: 'RAM' | 'STORAGE') => {
      const toggleMap = {
        RAM: () => setExpandableRAM(prev => !prev),
        STORAGE: () => setExpandableStorage(prev => !prev),
      } as const;

      toggleMap[type]();

      // If enabling filter without prior search, fetch all products
      const willEnable = type === 'RAM' ? !expandableRAM : !expandableStorage;
      if (willEnable && !hasSearched) {
        setHasSearched(true);
        mutate('');
      }
    },
    [expandableRAM, expandableStorage, hasSearched],
  );
  ...
  const displayedData = useMemo(() => {
    ...
    return filtered;
  }, [rawData, expandableRAM, expandableStorage]);
  ...
  }, [searchQuery, hasActiveFilter, hasSearched]);

  return (
    <AppLayout title="Product List" needBack needPadding>
      <FlatList
        ref={listRef}
